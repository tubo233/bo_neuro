...
# 创建所需的目录
os.makedirs('./bo_save_jpg/ROC/', exist_ok=True)
os.makedirs('./bo_save_csv/ROC/', exist_ok=True)
os.makedirs('./bo_save_model/LGBM/', exist_ok=True)
...
class LGBMClassifier:
    def __init__(self):
        self.data_types = {
            'frequency_distribution': {
                'file': 'bo_save_csv/data/process_frequency_distribution_median.tsv',
            ...
            'pac': {
                'file': 'bo_save_csv/data/process_pac_median.tsv',
            ...
            'symmetry': {
                'file': 'bo_save_csv/data/process_symmetry_median.tsv',
    ...
    def save_model_onnx(self, model, state, feature):
        model_file = f'./bo_save_model/LGBM/POD-noPOD_Classifier_{state}_{feature}.txt'
        onnx_file = f'./bo_save_model/LGBM/POD-noPOD_Classifier_{state}_{feature}.onnx'
    ...
    def plot_roc_curves(self, state, data_type, features):
        ...
        for i, feature in enumerate(features, 1):
            ...
            plt.title(f'{feature}', fontsize=17)
            plt.legend(loc="lower right", fontsize=13)
        
        plt.suptitle(f'ROC Curves for {state} ({data_type})', y=1.02, fontsize=24)
        plt.tight_layout()
        plt.savefig(f'./bo_save_jpg/ROC/{state}_Classifier_ROC_AUC_{data_type}.jpg', 
    ...
    def save_results_csv(self):
        ...
        df_results.to_csv('./bo_save_csv/ROC/POD-noPOD_Classifier_AUC_results.csv', 
                         index=False)
