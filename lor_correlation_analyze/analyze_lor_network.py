...
def plot_network_LOR_correlation(data_path, save_path):
    """
    ...
    for state in states:
        ...
        for feature in network_features:
            ...
            LOR_data = state_data['LORCeP'].values

            valid_indices = ~pd.isna(LOR_data) & ~pd.isna(feature_data)
            filtered_LOR_data = LOR_data[valid_indices]
            filtered_feature_data = feature_data[valid_indices]

            if len(filtered_LOR_data) < 2:
            ...
            # 绘制散点图和回归线
            sns.regplot(x=filtered_LOR_data, y=filtered_feature_data, 
            ...
            plt.title(f'LOR Correlation for {feature} ({state})', fontsize=16)
            plt.xlabel('LOR(CeProp)', fontsize=12)
            ...
            plt.savefig(os.path.join(save_dir, f'LOR_corr_{state}_{feature}.jpg'), dpi=300)
            plt.close()

if __name__ == "__main__":
    # 设置文件路径
    os.makedirs('bo_save_jpg/LORCorrelation', exist_ok=True)
    data_file_path = "bo_save_csv/data/process_network_median.tsv"
    save_imLOR_path = "./bo_save_jpg/LORCorrelation"
    
    plot_network_LOR_correlation(data_file_path, save_imLOR_path)
    print("Network LOR correlation analysis complete.")