...
class DataLoader:
    def __init__(self, data_path, data_type):
        ...
        self.feature_columns = {
            ...
            'network': ['small_worldness_alpha', 'global_efficiency_alpha', 
    ...
    def _get_time_periods(self, annotations):
        ...
        例如：对于两个连续的open-eyes，第一个的relative_time为start，第二个为end
        """
        periods = []
    ...
    def _process_data(self):
        ...
        for filename in unique_filenames:
            ...
            if self.data_type in ['frequency_distribution', 'pac', 'symmetry']:
                file_path = os.path.join(self.data_path, 'features-cerebral_raw_filtered_ref_average_eog_removed_motion_removed-10s', f"{filename}_{self.data_type}.tsv")
                ...
                for ch_name in data[ch_col].unique():
                    ...
                    for period in file_periods:
                        ...
                        # 移除异常值并计算中位数
                        period_data = self._remove_outliers(period_data, features)
                        ch_median = period_data[features].median()
                        
                        # 创建该通道的数据行
                        channel_row = pd.DataFrame({
                            ch_col: [ch_name],
                            **{col: [ch_median[col]] for col in features},
                            'state': [period['state']],
                            'filename': [filename]
                        })
                ...
                file_path = os.path.join(self.data_path, 
                                         'features-cerebral_raw_filtered_ref_average_eog_removed_motion_removed-20s', 
                                         f"{filename}_network_features.tsv")
                ...
                # 读取数据
                data = pd.read_csv(file_path, sep='\t')
                features = self.feature_columns['network']
                
                # 网络特征通常不分通道，是全局的，所以不需要按通道循环
                # 直接处理每个时间段
                for period in file_periods:
                    mask = (data['epoch_id'] >= period['start']) & (data['epoch_id'] <= period['end'])
                    ...
                    # 移除异常值并计算中位数
                    period_data = self._remove_outliers(period_data, features)
                    median_data = period_data[features].median()
                    
                    # 创建数据行
                    # 注意这里没有 ch_name 或 ch_col，因为是全局特征
                    channel_row = pd.DataFrame({
                        **{col: [median_data[col]] for col in features},
                        'state': [period['state']],
                        'filename': [filename]
                    })
                    channel_results.append(channel_row)
                ...
                # 获取该文件名对应的所有通道文件
                file_pattern = os.path.join(self.data_path, 'time_frequency-cerebral_raw_filtered_ref_average_eog_removed_motion_removed-10s', f"{filename}_*_psd.tsv")
...
if __name__ == "__main__":
    ...
    data_type = 'network'#frequency_distribution symmetry pac time-frequency network
    ...
    # 保存处理后的数据
    output_path = f'bo_save_csv/data/process_{data_type}_median.tsv'
