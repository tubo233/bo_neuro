...
class BrainTopographyPlotter:
    ...
    def run_analysis(self):
        """运行完整分析流程"""
        # 创建保存目录
        Path("./bo_save_jpg/Map/PAC/").mkdir(parents=True, exist_ok=True)
        Path("./bo_save_csv/Map/PAC/").mkdir(parents=True, exist_ok=True)
        
        # 读取数据
        df = self.load_and_process_data("bo_save_csv/data/process_pac_median.tsv")
        ...
        for state in ['open-eyes', 'close-eyes']:
            ...
            self.plot_topomap(
                ...
                f"./bo_save_jpg/Map/PAC/POD_vs_noPOD_{state}_pvalue_map_pac.jpg",
                'p_value'
            )
            ...
            self.plot_topomap(
                ...
                f"./bo_save_jpg/Map/PAC/POD_vs_noPOD_{state}_difference_map_pac.jpg",
                'difference'
            )
            ...
            self.plot_topomap(
                ...
                f"./bo_save_jpg/Map/PAC/{state}_age_correlation_map_pac.jpg",
                'correlation'
            )
        
        # 合并所有结果并保存到CSV
        all_results_df = pd.concat(all_results, ignore_index=True)
        all_results_df.to_csv("./bo_save_csv/Map/PAC/features_statistics_results.csv", index=False)
